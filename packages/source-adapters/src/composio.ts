// ComposioHQ/awesome-claude-skills mirror.
// Parses bullet-link entries from the README — 1000+ curated skills.
...
// Matches both plain and bold link entries:
//   - [name](url) - desc
//   - **[name](url)** - desc
const ENTRY_LINE =
  /^[-*]\s+(?:\*{1,2}|__)?\[([^\]]+)\]\(([^)]+)\)(?:\*{1,2}|__)?\s*[-—:]?\s*(.*)$/;
...
function isExternalLink(url: string): boolean {
  ...
  if (url.includes('makeapullrequest.com')) return false;
  return true;
}
...
export function createComposioAdapter(opts: ComposioAdapterOptions = {}): SourceAdapter {
  ...
  async function* fetchListings(): AsyncIterable<RawListing> {
    ...
    if (!res.ok) throw new Error(`ComposioHQ README fetch failed: HTTP ${res.status}`);
    const text = await res.text();
    const entries = parseReadme(text);
    for (const entry of entries) {
      yield {
        source: 'composio-mirror',
  ...
  return {
    ...
    normalize(raw): NormalizedListing {
      ...
      return {
        ...
        toolCompatibility: ['claude-code', 'cursor', 'codex-cli', 'gemini-cli'],
        category: e.category ?? 'development',
        tags: ['composio', 'curated', e.category ?? 'skill'].filter(Boolean),
        license: 'Apache-2.0',
        sourceType: 'composio-mirror' as any,
        sourceUrl: e.url,
        authorHandle: extractGitHubOwner(e.url) ?? 'composio',
        contentHash: sha256Hex(`composio:${slug}:${e.url}`),
      };
    },
  };
}
...
function parseReadme(markdown: string): Entry[] {
  ...
  for (const line of markdown.split(/\r?\n/)) {
    ...
    entries.push({
      name: name.trim(),
      url: url.trim(),
      description: desc?.replace(/\*By \[@.+$/, '').trim() ?? '',
